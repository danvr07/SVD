...
function new_X = task3 (photo, pcs)
  ...
  norm_photo = photo - row_means;
  
  % Calculate the covariance matrix.
  t_norm_photo = transpose(norm_photo);
  cov_mat = norm_photo * t_norm_photo / (n - 1);
  ...
  % asa mai departe.
  [val_proprii, indici] = sort(diag(D), 'descend');
  V = V(:, indici);
